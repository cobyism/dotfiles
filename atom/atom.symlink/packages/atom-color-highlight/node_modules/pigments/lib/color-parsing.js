(function() {
  ...
  module.exports = ColorParsing = (function(_super) {
    ...
    ColorParsing.scanBufferForColors = function(buffer, callback) {
      return this.scanBufferForColorsInRange(buffer, [[0, 0], [Infinity, Infinity]], callback);
    };

    ColorParsing.scanBufferForColorsInRange = function(buffer, range, callback) {
      var Color, Range, bufferText, defer, end, start, variablesPromise,
        _this = this;
      if (range == null) {
      ...
      variablesPromise = this.scanBufferForColorVariablesInRange(buffer, range);
      Color = this;
      variablesPromise.then(function(variablesMap) {
        var iterator, k, paletteRegexp, results, variables;
        variables = ((function() {
          var _results;
          _results = [];
        ...
        if (variables.length > 0) {
          ...
          Color.addExpression('variables', paletteRegexp, 1, function(color, expr) {
            return color.rgba = new Color(variablesMap[expr]).rgba;
          });
        }
        results = [];
        iterator = function(result) {
          var matchEnd, matchStart, _ref1;
          if (result != null) {
            _ref1 = result.range, matchStart = _ref1[0], matchEnd = _ref1[1];
            if (matchEnd <= end) {
    ...
    ColorParsing.prototype.parseExpression = function(colorExpression) {
